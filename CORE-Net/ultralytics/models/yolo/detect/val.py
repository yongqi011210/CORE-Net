...
class DetectionValidator(BaseValidator):
    ...
    def update_metrics(self, preds: list[dict[str, torch.Tensor]], batch: dict[str, Any]) -> None:
        ...
        for si, pred in enumerate(preds):
            ...
            # # 立刻打印 raw model pred（尚未 prepare 的）
            # print("[DEBUG] raw model pred (before _prepare_pred):", type(pred))
            # try:
            #     # if pred is tuple like (x_cat, (x_list, angle))
            #     if isinstance(pred, tuple):
            #         print("[DEBUG] raw pred tuple shapes:",
            #               getattr(pred[0], "shape", None),  # x_cat
            #               "inner:", type(pred[1]),
            #               getattr(pred[1][0], "shape", None) if isinstance(pred[1], tuple) else None)
            # except Exception:
            #     pass
            ...
            # print("[DEBUG] === update_stats 调用前 ===")
            # print(f"[DEBUG] predn type: {type(predn)}")
            # if isinstance(predn, dict):
            #     for k, v in predn.items():
            #         print(f"[DEBUG] predn[{k}] type: {type(v)}, shape: {getattr(v, 'shape', None)}")
            # else:
            #     print(f"[DEBUG] predn 内容: {predn}")
            #
            # print(f"[DEBUG] pbatch type: {type(pbatch)}")
            # print(f"[DEBUG] cls type: {type(cls)}, shape: {getattr(cls, 'shape', None)}")
            ...
            # print("[DEBUG] === update_stats 调用后 ===")
            # print(f"[DEBUG] no_pred: {no_pred}")
            # Evaluate
            if self.args.plots:
