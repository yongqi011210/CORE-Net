...
from ultralytics.nn.modules.PCR_Head import PCR_Head
import contextlib
import pickle
import re
import types
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            c2 = sum(ch[x] for x in f)
                ...
                {Detect, PCR_Head, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn,
                 v10Detect}
        ):
            args.append([ch[x] for x in f] if isinstance(f, (list, tuple)) else [ch[f]])

            if m in {Segment, YOLOESegment}:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)

            if m in {Detect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, PCR_Head}:
                m.legacy = legacy
