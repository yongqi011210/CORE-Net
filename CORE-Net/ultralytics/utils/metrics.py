...
def batch_probiou(obb1: torch.Tensor | np.ndarray, obb2: torch.Tensor | np.ndarray, eps: float = 1e-7) -> torch.Tensor:
    ...
    References:
        https://arxiv.org/pdf/2106.06072v1.pdf
    """
    # print("\n[DEBUG] ===== 进入 batch_probiou =====")
    # print(f"[DEBUG] obb1 type: {type(obb1)}, len: {len(obb1) if isinstance(obb1, (list, np.ndarray, torch.Tensor)) else 'N/A'}")
    # print(f"[DEBUG] obb2 type: {type(obb2)}, len: {len(obb2) if isinstance(obb2, (list, np.ndarray, torch.Tensor)) else 'N/A'}")
    #
    # # 检查前几项
    # if isinstance(obb1, (list, np.ndarray, torch.Tensor)):
    #     print(f"[DEBUG] obb1[0]: {obb1[0]}")
    # if isinstance(obb2, (list, np.ndarray, torch.Tensor)):
    #     print(f"[DEBUG] obb2[0]: {obb2[0]}")
